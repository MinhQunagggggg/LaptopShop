...
public class CustomerDAO {
   ...
   public List<User> searchCustomer(String keyword) {
    List<User> customers = new ArrayList<>();
    String query = "SELECT user_id, name, username, phone, email, avatar_data, shipping_address " +
                   "FROM Users " +
                   "WHERE role_id = 1 AND (name LIKE ? OR phone LIKE ? OR email LIKE ?)";

    try {
        ResultSet rs;
        try (PreparedStatement ps = new DBContext().getConnection().prepareStatement(query)) {
            ...
            while (rs.next()) {
                ...
                // ✅ Trả về dữ liệu ảnh dưới dạng byte[]
                byte[] avatarData = rs.getBytes("avatar_data");
                if (avatarData != null) {
                    customer.setAvatarUrl(avatarData);
                } else {
